...
export default (function store() {
    ...
    const populateDataByYear = (data) => {
      const result = {};
      data.forEach((item, index) => {
        item.uid = index;
        let [year, rank] = item['Top Ten'].split(' ');
        if (year && rank) {
          item.rankNum = Number.parseInt(rank.match(/[0-9]+/g)[0]);
          item.year = year;
          item.rank = rank;
          if (!result[year]) {
            result[year] = [];
      ...
      Object.keys(result)
        .forEach(key => {
          result[key].sort((x, y) => x.rankNum - y.rankNum);
        });
      return result;
    };
    ...
    const getCountriesList = (data) => {
      const countriesMap = {};
      data.forEach(item => {
        countriesMap[item.Country] = true;
      });
    ...
    return {
      getData: async function () {
          ...
          try {
              ...
              data = response.data;
              dataByYear = populateDataByYear(data);
              countries = getCountriesList(data);
              fetched = true;
              return Promise.resolve(data);
          } catch(ex) {
